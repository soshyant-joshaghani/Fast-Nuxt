#!/usr/bin/env python3
"""fast-nuxt SSH control — setup, clone, env, start/stop, update/reset, ACME.

Standalone (not foxg-ctrl). Run from this folder: fast-nuxt-ctrl.bat …
"""
...
def _add_target(p: argparse.ArgumentParser) -> None:
    p.add_argument(
        ...
        nargs="?",
        default="fast-nuxt",
        help="server id (default: fast-nuxt) or 'all'",
    )
    p.add_argument(
        "--all-disabled",
        action="store_true",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="fast-nuxt-ctrl",
        description=(
            "Single CLI for fast-nuxt: local dev, tests, prod helpers, and SSH VM ops."
        ),
        ...
        epilog=(
            ...
            "\n"
            "SSH (VM):\n"
            "  fast-nuxt-ctrl.bat setup | clone | env | start | stop | update\n"
            "  fast-nuxt-ctrl.bat backup-acme | status | connect"
        ),
    )
    ...
    for name, help_, fn in [
        ...
        ("clone", "Clone / pull the fast-nuxt repo on the VM", cmd_clone),
...
def _repl() -> int:
    parser = build_parser()
    echo("fast-nuxt-ctrl> (setup-local | dev run all | test all | list | quit)")
    while True:
        try:
            line = input("fast-nuxt-ctrl> ").strip()
