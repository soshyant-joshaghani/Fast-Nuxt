...
for img in fast-nuxt-backend:prod fast-nuxt-frontend:prod; do
...
#   FAST_NUXT_STORAGE=external bash __ctrl__/remote/start-prod.sh
if [[ -f compose.minio.yml && "${FAST_NUXT_STORAGE:-bundled}" != "external" ]]; then
...
echo
echo "fast-nuxt production stack started (includes ARQ worker)."
if [[ -f compose.minio.yml && "${FAST_NUXT_STORAGE:-bundled}" != "external" ]]; then
  ...
  echo "Object storage: external (FAST_NUXT_STORAGE=external — no compose.minio.yml)."
fi
