...
laptop CLI. Prefer **`fast-nuxt-ctrl`** from your machine when possible.

| Script | CLI equivalent |
|--------|----------------|
| `start-prod.*` | `fast-nuxt-ctrl start` (SSH) or `prod start` (local) |
...
| `setup-ubuntu.sh` | Prefer `fast-nuxt-ctrl setup` from laptop |
