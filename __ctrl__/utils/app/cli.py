...
<template>
  <Card>
    <CardHeader>
      <CardDescription class="uppercase tracking-[0.2em]">{title}</CardDescription>
      <CardTitle>{title}</CardTitle>
    </CardHeader>
    <CardContent>
      <p class="text-muted-foreground">
        Implement UI in
        <code class="text-foreground">lib/modules/apps/{name}/</code>.
        API base: <code class="text-foreground">{{ moduleUrl() }}</code>
      </p>
    </CardContent>
  </Card>
</template>
...
def _frontend_files(name: str) -> None:
    api_dir = PROJECT / "frontend/lib/modules/apps" / name
    _write_if_missing(api_dir / "api.ts", _frontend_api(name))
    _write_if_missing(
        PROJECT / "frontend/pages/(dashboard)" / name / "index.vue",
...
def cmd_app_create(args: argparse.Namespace) -> int:
    ...
    print(f"[fast-nuxt] Scaffolding app module: {name}")
    _backend_files(name)
    ...
    print(f"  3. Implement UI in frontend/pages/(dashboard)/{name}/")
    print(f"  4. Run: __ctrl__\\fast-nuxt-ctrl.bat test backend")
    return 0
