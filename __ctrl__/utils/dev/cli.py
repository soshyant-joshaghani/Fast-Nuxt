"""fast-nuxt-ctrl dev — run/stop local Docker Desktop / host app stacks."""
...
# __ctrl__ lives under fast-nuxt/; the compose + apps root is the parent.
PROJECT = ROOT.parent
...
COMPOSE_PROJECT_NAME = "fast-nuxt-dev"
...
APP_PORTS = (
    ("fast-nuxt-backend", 8000),
    ("fast-nuxt-worker", None),
    ("fast-nuxt-frontend", FRONTEND_DEV_PORT),
)
...
def _setup_local(*, force_install: bool = False) -> int:
    ...
    if not venv.is_dir():
        if not _require_cmd("python"):
            return 1
        print("[fast-nuxt] Creating Python venv at .venv ...")
    ...
    if created or force_install:
        print("[fast-nuxt] Installing Python requirements...")
        proc = subprocess.run(
    ...
    if not (PROJECT / "node_modules").is_dir() or force_install:
        print("[fast-nuxt] Installing npm dependencies at root ...")
    ...
    # Optional (nuxt prepare etc.); ignore failure if workspace script missing.
    _run_npm(
        "run",
        "prepare",
        "-w",
...
def _run_migrations() -> int:
    ...
    print("[fast-nuxt] Running migrations + seed (local prestart)...")
    for cmd in steps:
        print(f"  {' '.join(cmd)}")
    ...
    print("[fast-nuxt] Migrations + seed complete.")
    return 0
...
def _start_infra(*, slim: bool = False) -> int:
    ...
    print("[fast-nuxt] Waiting for Postgres...")
    if _wait_postgres("db") != 0:
        return 1

    if not slim:
        print("[fast-nuxt] Waiting for Redis...")
        if _wait_redis() != 0:
            return 1
...
def _start_named_console(
    ...
    *,
    ...
    """Open a titled console like start-fast-nuxt-dev.bat (`start "title" cmd /k …`).
...
def _start_apps(*, slim: bool = False) -> int:
    ...
    print(f"[fast-nuxt] Starting apps ({profile}): backend + frontend" + ("" if slim else " + worker"))
    if sys.platform == "win32":
    ...
    if not slim:
        print("Worker: fast-nuxt-worker (ARQ)")
    print("Stop with: fast-nuxt-ctrl.bat dev stop all")
    return 0


def _stop_apps() -> int:
    print("[fast-nuxt] Stopping host apps (backend / frontend)...")
    killed: set[int] = set()
...
def _compose_down(*, volumes: bool = False) -> int:
    ...
    if volumes:
        cmd.append("-v")
        print("[fast-nuxt] compose down -v  (WIPES VOLUMES)")
    code = _compose(*cmd)
    if code != 0:
        return code
    if volumes:
...
def _dev_help(args: argparse.Namespace) -> int:
    _ = args
    print(
        "usage: fast-nuxt-ctrl.bat dev {run|start,stop,down,purge,reset} {infra,apps,all}\n"
        "\n"
        "examples:\n"
        "  fast-nuxt-ctrl.bat dev run all\n"
        "  fast-nuxt-ctrl.bat dev run all --slim\n"
        "  fast-nuxt-ctrl.bat dev start all\n"
        "  fast-nuxt-ctrl.bat dev stop apps\n"
...
def cmd_setup_local(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "force", False):
        venv = PROJECT / ".venv"
        if venv.is_dir():
            print("[fast-nuxt] Removing existing .venv (--force)...")
            shutil.rmtree(venv)
        nm = PROJECT / "node_modules"
        if nm.is_dir():
            print("[fast-nuxt] Removing node_modules (--force)...")
            shutil.rmtree(nm)
