[![](https://github.com/soshyant-joshaghani/Fast-Nuxt/raw/main/Fast-System.png?raw=true)](https://github.com/soshyant-joshaghani/Fast-Nuxt)

[![](https://github.com/soshyant-joshaghani/Fast-Nuxt/raw/main/Fast-Nuxt.png?raw=true)](https://github.com/soshyant-joshaghani/Fast-Nuxt)
...
Fast-Nuxt is a **product-agnostic launchpad** — not a finished product and not a domain-specific template. Use the same foundation for dashboards, CRUD apps, APIs, SaaS products, 3D/WebGL experiences, background-processing services, or AI-powered applications. The difference is the **modules you add**, not a different project architecture.

**Documentation:** [AGENTS.md](AGENTS.md) · [ROADMAP.md](ROADMAP.md) · [docs/](docs/)

---

## What Fast-Nuxt provides

Fast-Nuxt answers repeated architectural and infrastructure questions before development starts:

| Pillar | What you get |
|--------|--------------|
| **Architecture** | Modular backend (FastAPI) + frontend (Nuxt) with clear layer boundaries |
...
You provide product requirements, features, business rules, UI needs, and integrations. Fast-Nuxt provides structure, conventions, lifecycle control, background processing, testing, and deployment.

> **Python backend + TypeScript frontend = Full Stack.** Nuxt + shadcn-vue for UI. FastAPI for API. PostgreSQL for data. Redis + ARQ for background jobs. `__ctrl__` for the workflow.

---

## Why it exists

Building a service usually means re-deciding the same things: project layout, module boundaries, database migrations, auth, background jobs, testing, and deployment. Fast-Nuxt makes those decisions once — so you and AI agents can focus on the product.

```text
Idea
  ↓
Fast-Nuxt (foundation + guardrails)
  ↓
...
Fast-Nuxt does not build products for you. It provides the runway; AI helps implement features **inside** the existing architecture.
...
| Layer | Tech | Dev URL |
|-------|------|---------|
| Frontend | Nuxt 3 + Vue 3 + shadcn-vue + TypeScript + Tailwind | http://dashboard.localhost |
...
|---------|---------|----------|
| **Full** | `dev run all` | Postgres, Redis, ARQ worker, Traefik, Adminer, uvicorn, Nuxt |
| **Slim** | `dev run all --slim` | Postgres, Traefik, Adminer, uvicorn, Nuxt (no Redis / no worker) |
...
| Direct Nuxt | http://localhost:3100 |
| Direct API | http://localhost:8000/docs |

Linux/mac:

```bash
chmod +x __ctrl__/fast-nuxt-ctrl.sh
__ctrl__/fast-nuxt-ctrl.sh setup-local
__ctrl__/fast-nuxt-ctrl.sh dev run all
```

Stop: `__ctrl__\fast-nuxt-ctrl.bat dev stop all`

**Port 80/443 conflict:** only one Traefik-on-`:80` stack at a time. Stop the other proxy or run apps only: `dev run apps` (direct `http://localhost:3100` / `http://localhost:8000/docs`).
...
2. Scaffold (recommended): `__ctrl__\fast-nuxt-ctrl.bat app create myfeature`
3. Backend: `backend/app/modules/apps/<name>/`
4. Frontend client: `frontend/lib/modules/apps/<name>/api.ts`
5. Nuxt route: `frontend/pages/(dashboard)/<path>/index.vue`
...
Under the frontend modules root (`frontend/lib/modules/`) there are **only**:
...
├── frontend/
│   ├── pages/                # Nuxt routes (login, (dashboard)/)
│   ├── composables/          # useAuth, etc.
│   └── lib/
│       ├── config/           # API_BASE_URL
│       └── modules/
│           ├── base/         # kit/platform (auth, shell) + ui/
│           └── apps/         # feature API clients
└── tests/
...
| **Fast-Nuxt** (this repo) | General-purpose foundation (Nuxt UI) |
...
`NUXT_PUBLIC_API_BASE_URL` — where the frontend calls FastAPI (browser).
