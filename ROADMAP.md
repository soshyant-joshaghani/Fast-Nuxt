# Fast-Nuxt — AI & Developer Foundation Roadmap

## 1. Project Vision

Fast-Nuxt is an opinionated, modular, full-stack foundation designed to help developers go from an idea to a deployable service with minimal infrastructure decisions.

The goal is NOT to build a template for a specific type of product.

Fast-Nuxt must remain product-agnostic.
...
The difference between these products should primarily be the modules implemented on top of Fast-Nuxt, not a completely different project architecture.

The core philosophy is:

> Python backend + TypeScript frontend = Full Stack

A developer should be able to use Python for the backend, business logic, database layer, background processing, testing, and deployment workflow — and TypeScript/Vue for a modern, ecosystem-rich frontend.

Nuxt 3 + shadcn-vue provides the application UI.
...
Fast-Nuxt is intended to be a launchpad, not a finished product.

---

# 2. Primary Goal

The primary goal is to make Fast-Nuxt an excellent foundation for AI-assisted development and Vibe Coding.
...
All of these should be possible applications built on top of Fast-Nuxt.
...
Fast-Nuxt intentionally makes architectural decisions.
...
Conceptually:
    ...
    frontend/lib/modules/apps/<feature>/
    frontend/pages/<feature>/...
...
Nuxt 3 + Vue 3 + shadcn-vue is a first-class part of Fast-Nuxt.

The frontend should not be treated as an afterthought.

Fast-Nuxt pairs a Python backend with a modern TypeScript/Vue frontend — giving access to the full npm ecosystem (Three.js, Babylon.js, charting libraries, etc.) while keeping backend conventions in Python.
...
Redis and ARQ are part of the official Fast-Nuxt infrastructure.

They should NOT be described as "optional technology" or as an unrelated add-on.

Fast-Nuxt officially supports two runtime profiles:
...
Fast-Nuxt also officially supports a lightweight development/runtime mode.
...
> Full is the complete Fast-Nuxt runtime.

> Slim is the lightweight Fast-Nuxt runtime.

Slim mode is NOT a workaround.

Slim mode is NOT a fallback.

Slim mode is an official supported operating mode of Fast-Nuxt.
...
This document must explain how AI agents are expected to work inside Fast-Nuxt.
...
The AI should not redesign Fast-Nuxt simply because it personally prefers another architecture.

---

# 9. Canonical Example Module

Fast-Nuxt should contain one intentionally simple but complete example module.
...
> "This is how a feature is implemented in Fast-Nuxt."
...
Fast-Nuxt should remain usable from the first local development command through production deployment.
...
> Does this make Fast-Nuxt a better general-purpose foundation for developers and AI agents?
...
Product-specific functionality belongs in application modules, not the Fast-Nuxt core.

---

# 22. Acceptance Criteria for Fast-Nuxt

Fast-Nuxt should eventually satisfy the following scenario:
...
Fast-Nuxt should feel like a launchpad.
