...
Fast-Nuxt is a modular full-stack foundation. This document describes how the pieces fit together.
...
|-------|------------|------|
| Frontend | Nuxt 3 + Vue 3 + shadcn-vue + TypeScript + Tailwind | UI components, routes, API clients |
...
Fast-Nuxt supports two official dev/runtime modes:

| Profile | Command | Infrastructure | Apps |
|---------|---------|----------------|------|
| **Full** | `dev run all` | DB + Redis + Traefik + Adminer | uvicorn + ARQ worker + Nuxt |
| **Slim** | `dev run all --slim` | DB + Traefik + Adminer | uvicorn + Nuxt |
...
├── frontend/
│   ├── pages/                # Nuxt routes (login.vue, (dashboard)/)
│   ├── components/
│   │   ├── layout/           # Dashboard shell (sidebar, header)
│   │   └── ui/               # shadcn-vue primitives
│   ├── composables/          # useAuth.ts and shared composables
│   ├── lib/
│   │   ├── config/           # API URL helpers (backend.ts, api-url.ts)
│   │   └── modules/
│   │       ├── global/       # Auth fetch helpers, shared utils
...
Shared infrastructure used by every application: authentication, users. These ship with Fast-Nuxt and are not product-specific.

Uses `crud.py` in the users submodule (predates the app-module convention).

### App modules (`modules/apps/<name>/`)

Product features you build on top of Fast-Nuxt. Each module owns its backend and frontend code.
...
```
frontend/lib/modules/apps/<name>/
└── api.ts          # TypeScript HTTP client

frontend/pages/(dashboard)/<path>/index.vue   # Nuxt route
```
...
| Auth | `frontend/composables/useAuth.ts` |
...
- Frontend: `useAuth()` composable persists token in localStorage
...
| Frontend API URL | `NUXT_PUBLIC_API_BASE_URL` env var |
...
```bat
__ctrl__\fast-nuxt-ctrl.bat test backend
```

Frontend tests use Vitest:

```bat
__ctrl__\fast-nuxt-ctrl.bat test frontend
```
...
Creates backend router, frontend `api.ts` stub, Nuxt route stub, and test — then extend using the sample module as reference.

## What belongs in core vs apps

**Core** (Fast-Nuxt foundation): auth, database, migrations, CLI, deployment, testing infrastructure, modular conventions.
...
Fast-Nuxt intentionally uses the **simple approach**: services raise `HTTPException` for business errors. No separate application-exception hierarchy.
...
Fast-Nuxt does **not** ship a storage abstraction yet. When applications need uploads or generated files:
...
| [dashboard-ui.md](dashboard-ui.md) | Dashboard shell and shadcn-vue |
