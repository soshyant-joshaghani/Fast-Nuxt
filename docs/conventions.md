...
Naming, layers, and cross-module rules for Fast-Nuxt. Follow these in app modules; platform code (`base/`, `system/`) sets the baseline.
...
| Vue SFC files | `PascalCase.vue` for components | `LoginForm.vue`, `AppSidebar.vue` |
...
| Composables | `use` prefix, `camelCase` file | `useAuth.ts`, `useAuth()` |
...
| Nuxt routes | folder-based paths under `pages/` | `pages/(dashboard)/sample/notes/index.vue` |
...
Use `useAuth()` for JWT token and user state — do not duplicate auth storage.
...
Fast-Nuxt uses **standard FastAPI**, not a custom envelope.
...
**Intentional decision:** Fast-Nuxt uses the simple approach — services raise `HTTPException` directly. Do not introduce a separate application-exception hierarchy unless a product module explicitly requires it.

Do **not** introduce FoxG-style `{ "code": "SUCCESS", "data": {}, "meta": {} }` wrappers unless an app module explicitly requires that contract.

## Cross-module relations

Fast-Nuxt modules: `base/` (platform), `system/`, `apps/<feature>/`.
...
- Frontend: `useAuth()` composable + `middleware/auth.global.ts`
...
These belong in **app modules**, not Fast-Nuxt foundation docs or core code:
