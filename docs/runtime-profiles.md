...
fast-nuxt officially supports two **development** runtime profiles. Both are valid — neither is a workaround or degraded mode.

Redis and ARQ are part of the full fast-nuxt runtime. Slim is an official lightweight profile for apps that do not need background processing — not because Redis is "optional infrastructure."
...
**Apps (host):** uvicorn (API :8000), ARQ worker, Nuxt (dashboard :3000)

Use when the application needs background jobs, or when you want the complete fast-nuxt stack locally.
...
**Apps (host):** uvicorn, Nuxt — **no ARQ worker**
